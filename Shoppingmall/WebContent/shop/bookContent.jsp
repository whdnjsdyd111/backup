...
<body>
	...
	<div id="showBook">
		<table class="vhcenter">
			...
			<tr><td width="500">저자 : ${ book.getAuthor() }</td>
			...
			<tr><td width="500">
					...
					<strong class="bred">판매가:<c:set var="rPrice" value="${ price * ((100.0 - rate) / 100) }"></c:set>
				...
				<ul>
					...
					<li>
						<c:if test="${ sessionScope.id == writer }">
							...
							<button id="edit" name="${ qna.getQna_id() },${ book_id },${ book_kind }" onclick="del(this)">삭제</button>
						</c:if>
				</ul>
			</c:forEach>
		</c:if>
